...
| R26 | **The top bar shows the logo of the product, `site/marca.webp`, drawn from `enqlagastan-logo.svg`.** It replaces the plain text of R20 inside the same link. The logo keeps its own colours; the theme does not tint it. | The text "En qué la gastan" alone | owner feedback, 2026-09-18 | - |
| R27 | **A new place, P5 "Más info" (`#/info`), reached by an icon button of a circle with an "i", after "Tema" and "Billetes", `aria-label="Más información"`, hit area 44x44px or more, no word.** Like P4 (D6), it carries no exercise: the year is empty and both year arrows are disabled. The chart pane states the goal and lists what comes next; the tape pane links to the code on GitHub, to X, and to P4. | A page outside the frame. A link in the foot of the tape only | owner feedback, 2026-09-21 | - |
...
- Corrected on 2026-09-21, from owner feedback: added R27, the place P5 "Más info".
